...
6️⃣ Confirm that everything is where it should be by checking the file structure.  
Type: tree 2>/dev/null || ls -R

💡 Why: "tree 2>/dev/null || ls -R" tries to show your folder structure in a nice tree format, and if the tree command isn’t available, falls back to ls -R so your project structure is displayed reliably on any system.

✨ Tip: If you want the tree command available in Git Bash, you can install it using a package manager. For example, you can type winget install GnuWin32.Tree and then restart Git Bash. After installation, you will only need the tree command to display the folder structure wherever you are along your path.
